...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="justinjustdoit/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/hazoon37/guestbook.git'
            }
        }
        stage('Build') {
            steps {
            ...
            steps{
                ...
                withSonarQubeEnv('SonarQube-Server'){
                    sh '''
                        ...
                        -Dsonar.host.url=http://13.125.11.142:9000 \
                        -Dsonar.login=17f3f1a01540bdd02c6dfd66af337cc5e7a4b210
                    '''
                }
                */
            }
        ...
        stage('Staging Deploy') {
            steps {
                sshagent(credentials: ['Staging-PrivateKey']) {
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@43.201.63.0 sudo docker container rm -f guestbookapp"
                    sh "ssh -o StrictHostKeyChecking=no ec2-user@43.201.63.0 sudo docker container run \
                                        -d \
                                        -p 38080:80 \
                                        --name=guestbookapp \
